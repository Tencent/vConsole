	...
	var VConsoleDefaultTab = function (_VConsoleLogTab) {
	  ...
	  _createClass(VConsoleDefaultTab, [{
	    ...
	    value: function onReady() {
	      ...
	      function retrievePrecedingIdentifier(text, pos, regex) {
	        ...
	        if (buf.length == 0) {
	          ...
	          for (var _i2 = pos - 1; _i2 >= 0; _i2--) {
	            if (regex.test(text[_i2])) buf.push(text[_i2]);else break;
	          }
	        }
	        if (buf.length == 0) {
	      ...
	      _query2.default.one('.vc-cmd-input').onkeyup = function (e) {
	        if (e.keyCode == 8 || e.keyCode == 46) {
	          return;
	        }
	        var prompted = _query2.default.one('.vc-cmd-prompted');
	        ...
	        if (value && value.length > 0) {
	          if (/\(/.test(value)) {
	            _query2.default.one('.vc-cmd-input').value += ')';
	            return;
	          }
	          if (/\[/.test(value)) {
	            _query2.default.one('.vc-cmd-input').value += ']';
	          ...
	          if ('.' == value) {
	            var key = retrievePrecedingIdentifier(temp_value, temp_value.length - 1);
	            if (!cache_obj[key]) {
	              try {
	            ...
	            try {
	              for (var _i3 = 0; _i3 < cache_obj[key].length; _i3++) {
	                var li = document.createElement('li');
	                li.setAttribute('style', ' border-bottom: solid 1px');
	                var _key = cache_obj[key][_i3];
	                li.innerHTML = _key;
	                li.onclick = function () {
	                  _query2.default.one('.vc-cmd-input').value = '';
	                  _query2.default.one('.vc-cmd-input').value = temp_value + this.innerHTML;
	                  prompted.style.display = 'none';
	                };
	            ...
	            for (var _i4 = 0; _i4 < winKeys.length; _i4++) {
	              if (winKeys[_i4].toLowerCase().indexOf(value.toLowerCase()) >= 0) {
	                ...
	                _li.setAttribute('style', ' border-bottom: solid 1px');
	                _li.innerHTML = winKeys[_i4];
	                _li.onclick = function () {
	                  _query2.default.one('.vc-cmd-input').value = '';
	                  _query2.default.one('.vc-cmd-input').value = this.innerHTML;
	                  if (keyTypes[this.innerHTML] == 'function') {
	                    _query2.default.one('.vc-cmd-input').value += '()';
	                  }
	                  prompted.style.display = 'none';
	                };
	                prompted.appendChild(_li);
	              }
	            }
	          } else {
	            var arr = value.split('.');
	            ...
	            if (cache_obj[arr[0]]) {
	              ...
	              for (var _i5 = 0; _i5 < cache_obj[arr[0]].length; _i5++) {
	                var _li2 = document.createElement('li');
	                _li2.setAttribute('style', ' border-bottom: solid 1px');
	                var _key4 = cache_obj[arr[0]][_i5];
	                if (_key4.indexOf(arr[1]) >= 0) {
	                  _li2.innerHTML = _key4;
	                  _li2.onclick = function () {
	                    _query2.default.one('.vc-cmd-input').value = '';
	                    _query2.default.one('.vc-cmd-input').value = temp_value + this.innerHTML;
	                    prompted.style.display = 'none';
	                  };
	                  prompted.appendChild(_li2);
	                }
	              }
	            }
	          }
